...
class OrdersTestCases(BaseTest):
    ...
    def test020_update_material_type_with_test_unit(self, save):
        ...
        self.order_page.set_test_unit(test_unit='t')
        self.order_page.save(save_btn='order:save_btn')
        self.base_selenium.get(url=order_url, sleep=self.base_selenium.TIME_MEDIUM)
        self.order_page.get_orders_page()
        ...
        self.base_selenium.LOGGER.info(' + order_url : {}'.format(order_url))
        order_material_type = self.order_page.get_material_type()
        new_material_type = self.order_page.set_material_type(material_type='Final Product')
        self.order_page.confirm_popup(force=True)
        self.order_page.set_article(article='d83e197955')
        self.order_page.set_test_unit(test_unit='tu_rm_ql')
        self.order_page.get_suborder_table()
        if 'save_btn' == save:
            self.order_page.save(save_btn='order:save_btn')
        else:
