...
class OrdersTestCases(BaseTest):
    ...
    def test066_Navigate_to_merged_page_after_clicking_on_edit_icon_of_main_order(self) :
        """
        LIMS-5371- Make Sure that when user click on edit icon he will be redirect to the first step
        ...
        self.info('press on edit icon')
        self.orders_page.get_order_edit_page_by_id(random_order['id'])
        order_url = self.base_selenium.get_url()

        self.info('check that it navigated to the right merged page that has the order data')
        self.assertEqual(order_url, self.orders_page.orders_url + '/' + str(random_order['id']))
